...
namespace BySLib.Utilities
{
    //ejemplo
    /*
     * if (!NumeroNif.CompruebaNif(this.tbDNI.Text.Trim()).EsCorrecto ||
                NumeroNif.CompruebaNif(this.tbDNI.Text.Trim()).TipoNif == "CIF")
                throw new Exception("El dni introducido no es correto.");
     * 
     *  En el validate data
     * 
     * */
